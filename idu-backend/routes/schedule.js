...
router.get(
  ...
  async (req, res) => {
    ...
    // Students see only their faculty schedule
    ...
    const { rows } = await db.query(
      `SELECT sc.id, sc.weekday, sc.start_time, sc.end_time,
              ...
              sc.room, sc.faculty, sc.year_of_study
       FROM schedule sc
...
router.post(
  ...
  [
    ...
    body('weekday').isInt({ min: 0, max: 6 }).toInt().withMessage('0=Mon … 6=Sun'),
  ...
  async (req, res) => {
    ...
    const { rows } = await db.query(
      `INSERT INTO schedule (course_id, weekday, start_time, end_time, room, faculty, year_of_study)
...
router.delete(
  ...
  async (req, res) => {
    const { rowCount } = await db.query(
      'UPDATE schedule SET is_active = FALSE WHERE id = $1',
      [req.params.id]
    );
