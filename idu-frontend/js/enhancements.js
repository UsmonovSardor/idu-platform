...
/* ═══════════════════════════════════════════════════════════════════
   ...
   ✅ Sayt ochilganda login modal avtomatik chiqmaydi
   ✅ Login faqat user "Kirish" tugmasini bosganda chiqadi
   ✅ URL role hint saqlangan, lekin modalni majburlab ochmaydi
   ✅ Proktor login patch saqlangan
   ✅ Dekanat navidan "Sesiya Boshqaruvi" olib tashlanadi
═══════════════════════════════════════════════════════════════════ */


// ── 0. BACKEND URL tuzatish ─────────────────────────────────────
(function fixBackendURL() {
  if (
    ...
    window.BACKEND_URL = 'https://idu-platform-production.up.railway.app';
  }

  if (typeof window.API_BASE === 'undefined' || !window.API_BASE) {
    window.API_BASE = 'https://idu-platform-production.up.railway.app/api';
  }
})();


// ── 1. Landing page yashirilmasin ────────────────────────────────
// Oldingi kod .hemis-hero va nav linklarni yashirib yuborgan.
// Endi sayt normal ochiladi.
...
(function urlRouting() {
  ...
  if (/dekanat/.test(p)) {
    window._IDU_URL_ROLE = 'dekanat';
  } else if (/professor|\/teacher/.test(p)) {
    window._IDU_URL_ROLE = 'teacher';
  } else if (/investor/.test(p)) {
    window._IDU_URL_ROLE = 'investor';
  } else if (/proktor/.test(p)) {
    window._IDU_URL_ROLE = 'proktor';
  }
})();
...
// ── 4. Login modal avtomatik ochilmasin ──────────────────────────
// MUHIM FIX:
// Bu yerda openLoginModal() avtomatik chaqirilmaydi.
// Modal faqat index/main.js dagi "Kirish" tugmasi bosilganda ochilishi kerak.
...
function _applyUrlRoleHint(role) {
  ...
  if (lEl && role !== 'proktor') {
    lEl.placeholder = role.charAt(0).toUpperCase() + role.slice(1) + ' logini';
  }
}


// ── 5. Login modal ochilganda URL role hint qo‘llash ─────────────
(function patchOpenLoginModalForHint() {
  function patch() {
    if (typeof window.openLoginModal !== 'function') {
      return setTimeout(patch, 300);
    }

    if (window.openLoginModal._iduHintPatched) return;

    var originalOpenLoginModal = window.openLoginModal;

    window.openLoginModal = function () {
      var result = originalOpenLoginModal.apply(this, arguments);
    ...
    window.openLoginModal._iduHintPatched = true;
  }
...
// ── 6. Login patch: proktor qo‘llab-quvvatlash ───────────────────
(function patchLoginForProktor() {
  function patch() {
    ...
    if (window.realAutoLogin._iduProktorPatched) return;

    var originalRealAutoLogin = window.realAutoLogin;
    ...
    window.realAutoLogin = async function () {
      ...
      return originalRealAutoLogin.apply(this, arguments);
    };

    window.realAutoLogin._iduProktorPatched = true;
  }
...
function _loginAsProktor() {
  ...
  if (typeof _ssSet === 'function') {
    _ssSet('idu_active_role', 'PRK_5xWt2nMv');
  }

  if (typeof saveSession === 'function') {
    saveSession('proktor', _PROKTOR);
  }

  if (typeof closeLoginModalForce === 'function') {
  ...
  if (typeof setupChip === 'function') {
    setupChip('proktor', _PROKTOR);
  }
...
// ── 7. Proktor logout patch ──────────────────────────────────────
(function patchLogout() {
  function patch() {
    if (typeof window.logout !== 'function') {
      return setTimeout(patch, 400);
    }

    if (window.logout._iduProktorLogoutPatched) return;
    ...
    window.logout = function () {
      if (window.currentRole === 'proktor') {
        ...
        try {
          localStorage.removeItem('idu_session');
          sessionStorage.removeItem('idu_active_role');
        } catch (e) {}
    ...
    window.logout._iduProktorLogoutPatched = true;
  }
...
// ── 8. Dekanat navidan "Sesiya Boshqaruvi" ni olib tashlash ──────
(function removeDekanatSesiyaTab() {
  function remove() {
    if (typeof NAV_TABS === 'undefined') {
      return setTimeout(remove, 400);
    }
...
function _renderProktorDashboard() {
  ...
  var etirazCount = _getEtirozList().filter(function (e) {
    return e.status === 'pending';
  }).length;

  var totalTest = _getEtirozList().filter(function (e) {
    return e.type === 'etiraz';
  }).length;
...
function _buildModeCard(type, active) {
  ...
  return [
    '<div style="background:#fff;border:2px solid ' + borderC + ';border-radius:16px;padding:24px;box-shadow:0 2px 12px rgba(0,0,0,0.04);transition:all 0.3s">',
      ...
      active
        ? '<div style="height:4px;background:#DCFCE7;border-radius:4px;margin-bottom:14px;overflow:hidden"><div style="height:100%;width:100%;background:linear-gradient(90deg,#16A34A,#22C55E);border-radius:4px;animation:progressPulse 2s ease-in-out infinite"></div></div>'
        : '<div style="height:4px;background:#F1F5F9;border-radius:4px;margin-bottom:14px"></div>',

      '<div style="display:flex;gap:10px">',
        active
          ? '<button onclick="proktorSetState(\'' + type + '\',false)" style="flex:1;padding:11px;background:linear-gradient(135deg,' + gradClose + ');color:white;border:none;border-radius:9px;font-family:\'Outfit\',sans-serif;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 3px 10px rgba(71,85,105,0.25)">🔒 Qulflash</button>'
          : '<button onclick="proktorSetState(\'' + type + '\',true)" style="flex:1;padding:11px;background:linear-gradient(135deg,' + gradOpen + ');color:white;border:none;border-radius:9px;font-family:\'Outfit\',sans-serif;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 3px 10px rgba(22,163,74,0.25)">✅ Ochish</button>',
      '</div>',
    '</div>'
  ].join('');
}
...
// ── 12. Proktor: sesiya/test holatini o‘zgartirish ───────────────
window.proktorSetState = function (type, active) {
  if (typeof SESIYA_STATE !== 'undefined') {
    SESIYA_STATE[type] = active;
  }

  if (typeof setSesiyaState === 'function') {
    setSesiyaState(type, active);
  }
  ...
  _addProktorLog(
    ...
    label + ' ' + action,
    time,
    active ? '#DCFCE7,#86EFAC,#16A34A' : '#FEE2E2,#FCA5A5,#DC2626'
  );

  if (typeof showToast === 'function') {
    showToast(emoji, label, active ? 'Talabalar kira oladi!' : 'Qulflandi. Talabalar kira olmaydi.');
  }
...
function _addProktorLog(emoji, msg, time, colors) {
  ...
  entry.style.cssText =
    'display:flex;align-items:center;gap:12px;padding:11px 14px;background:' +
    parts[0] +
    ';border-radius:10px;border:1px solid ' +
    (parts[1] || '#E2E8F0') +
    ';font-size:13px;animation:fadeSlide 0.3s ease';
...
window._clearProktorLog = function () {
  var log = document.getElementById('proktorLog');
  if (log) {
    log.innerHTML = '<div style="text-align:center;padding:24px;color:#94A3B8;font-size:13px">Hali harakatlar yo\'q</div>';
  }
};


// ── 13. Natijalar sahifasi ───────────────────────────────────────
function _renderProktorResults() {
  ...
  var rows = students.map(function (s) {
    ...
    var grade =
      avg >= 86 ? { t: 'A\'lo', c: '#16A34A', bg: '#DCFCE7' } :
      avg >= 71 ? { t: 'Yaxshi', c: '#1D4ED8', bg: '#DBEAFE' } :
      avg >= 56 ? { t: 'Qoniqarli', c: '#D97706', bg: '#FEF3C7' } :
                  { t: 'Qoniqarsiz', c: '#DC2626', bg: '#FEE2E2' };

    var attColor = s.att >= 90 ? '#16A34A' : s.att >= 80 ? '#D97706' : '#DC2626';
    ...
    return (
      '<tr style="border-bottom:1px solid #F8FAFC;transition:background 0.15s" onmouseover="this.style.background=\'#F8FAFC\'" onmouseout="this.style.background=\'\'">' +
        '<td style="padding:12px 16px">' +
          '<div style="display:flex;align-items:center;gap:10px">' +
            '<div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#1B4FD8,#3B82F6);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:white">' +
              String(s.name || 'T').split(' ').map(function (x) { return x[0] || ''; }).join('').slice(0, 2) +
            '</div>' +
            '<div><div style="font-weight:700;font-size:13.5px">' + (s.name || 'Noma\'lum') + '</div><div style="font-size:11px;color:#94A3B8">' + (s.group || '—') + '</div></div>' +
          '</div>' +
        '</td>' +
        ...
        '<td style="padding:12px 16px;text-align:center;font-family:\'DM Mono\',monospace;font-size:16px;font-weight:900;color:#1B4FD8">' + avg + '</td>' +
        ...
        '<td style="padding:12px 16px;text-align:center">' +
          '<div style="display:flex;align-items:center;justify-content:center;gap:6px">' +
            '<div style="flex:1;height:6px;background:#F1F5F9;border-radius:3px;max-width:60px;overflow:hidden">' +
              '<div style="height:100%;border-radius:3px;background:' + attColor + ';width:' + (s.att || 0) + '%"></div>' +
            '</div>' +
            '<span style="font-weight:700;color:' + attColor + ';font-size:13px">' + (s.att || 0) + '%</span>' +
          '</div>' +
        '</td>' +
  ...
  var avgScore = students.length
    ? Math.round(students.reduce(function (sum, x) { return sum + (x.avg || 0); }, 0) / students.length)
    : 0;

  var passPct = students.length
    ? Math.round(students.filter(function (s) { return (s.avg || 0) >= 56; }).length / students.length * 100)
    : 0;
...
function _renderProktorEtirozlar() {
  ...
  function eRow(e) {
    var stC =
      e.status === 'approved' ? { t: '✅ Tasdiqlandi', c: '#16A34A', bg: '#DCFCE7' } :
      e.status === 'rejected' ? { t: '❌ Rad etildi', c: '#DC2626', bg: '#FEE2E2' } :
                                { t: '⏳ Kutilmoqda', c: '#D97706', bg: '#FEF3C7' };

    return (
      '<div style="background:#fff;border:1.5px solid ' + (e.status === 'pending' ? '#FED7AA' : '#E2E8F0') + ';border-radius:14px;padding:18px 20px;margin-bottom:10px;transition:all 0.2s">' +
          ...
          '<div style="flex:1;min-width:0">' +
            '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' +
              '<div style="width:34px;height:34px;border-radius:9px;background:linear-gradient(135deg,#7C3AED,#A78BFA);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:white;flex-shrink:0">' +
                String(e.studentName || 'T').charAt(0) +
              '</div>' +
              '<div>' +
            ...
            '<div style="font-size:13px;font-weight:600;color:#1B4FD8;margin-bottom:6px">' + String(e.detail || '—').substring(0, 80) + '</div>' +

            (e.note
              ? '<div style="background:#FFF7ED;border:1px solid #FED7AA;border-radius:8px;padding:10px 12px;font-size:12.5px;color:#92400E;line-height:1.6"><strong>E\'tiroz sababi:</strong> ' + e.note + '</div>'
              : '') +
          '</div>' +
  ...
  page.innerHTML = [
    '<div style="padding:28px 32px;max-width:900px">',
      ...
      pending.length > 0
        ? '<div style="margin-bottom:22px"><div style="font-size:14px;font-weight:800;color:#0F172A;margin-bottom:12px;display:flex;align-items:center;gap:8px"><span style="padding:3px 10px;background:#FEF3C7;color:#D97706;border-radius:20px;font-size:12px">⏳ ' + pending.length + ' ta</span> Kutilayotgan e\'tirozlar</div>' + pending.map(eRow).join('') + '</div>'
        : '',

      resolved.length > 0
        ? '<div><div style="font-size:14px;font-weight:800;color:#64748B;margin-bottom:12px;display:flex;align-items:center;gap:8px"><span style="padding:3px 10px;background:#DCFCE7;color:#16A34A;border-radius:20px;font-size:12px">✅ ' + resolved.length + ' ta</span> Hal qilingan e\'tirozlar</div>' + resolved.map(eRow).join('') + '</div>'
        : '',
...
window._proktorApprove = function (id) {
  var app = typeof APPLICATIONS !== 'undefined' && APPLICATIONS.find(function (a) {
    return a.id === id;
  });
  ...
  if (typeof showToast === 'function') {
    showToast('✅', 'Tasdiqlandi', 'E\'tiroz tasdiqlandi');
  }
};
...
window._proktorReject = function (id) {
  ...
  if (typeof showToast === 'function') {
    showToast('❌', 'Rad etildi', 'E\'tiroz rad etildi');
  }
};


// ── 15. CSS animatsiyalar ────────────────────────────────────────
(function addStyles() {
...
console.log('[IDU Enhancements FIXED] ✓ Loaded — Login auto-open disabled');
