'use strict';
// ==============================================
//  IDU Platform — core/auth.js
//  Login, logout, OTP, session management
// ==============================================
...
async function verifyLoginOTP() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'â³ Tekshirilmoqda...'; }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (btn) { btn.disabled = false; btn.textContent = 'â Tasdiqlash va kirish'; }
      return;
    }
    // Muvaffaqiyat
    ...
    // Backend yo'q â lokal demo tekshirish
    if (entered !== _otpCode) {
      document.getElementById('otpError').classList.add('show');
      document.getElementById('otpInput').value = '';
      document.getElementById('otpInput').focus();
...
function loginStudent() {
  ...
  if (!phone || phone.length < 9) { errEl.querySelector('span').textContent='ð± Telefon raqamni to\'g\'ri kiriting'; errEl.classList.add('show'); return; }
  ...
  apiLogin('student', email, p, remember).then(function(res) {
    ...
    if (res.ok) {
      ...
      // Offline fallback â use local USERS
    ...
    _setLoginLoading('sLoginBtn', false);
    // Network error â use local fallback
    const override = getEffectivePass('student', l);
    const u = USERS.student.find(x=>x.login===l&&(override?override===p:x.pass===p));
    if (!u) { recordFailedAttempt('student:'+l); errEl.querySelector('span').textContent='Login yoki parol noto\'g\'ri'; errEl.classList.add('show'); return; }
    clearAttempts('student:'+l); errEl.classList.remove('show');
...
function generateNewCredentialsLocal() {
  ...
  showToast('â', 'Ma\'lumotlar tayyor', 'Yangi login va parol tayyor!');
}
