'use strict';
// IDU Platform — features/assignments.js
    ...
    <div class="task-item">
      <div class="task-top">
        <div>
          ...
          <div class="task-due">ð ${t.due} Â· ${t.pts} ball</div>
        </div>
        <span class="task-badge tb-${t.type}">${currentLang==='ru'?{test:'Ð¢ÐµÑÑ',lab:'ÐÐ°Ð±',hw:'Ð/Ð',project:'ÐÑÐ¾ÐµÐºÑ'}[t.type]:{test:'Test',lab:'Lab',hw:'Uy',project:'Loyiha'}[t.type]}</span>
      </div>
    </div>`).join('');
}
    ...
    <div class="task-item">
      <div class="task-top">
        <div>
          <div class="task-subject" style="color:var(--primary)">${t.sub}</div>
          <div class="task-name">${t.name}</div>
          <div class="task-due">ð Muddat: ${t.due}</div>
