'use strict';
// IDU Platform — features/grades.js
...
function loadGradeGroup(){
  ...
  if(tEl)tEl.textContent='âï¸ '+_curSub+' â '+_curGrp+' guruh';
  ...
  el.innerHTML=students.map(function(s,i){
    ...
    return '<tr id="grade-row-'+s.id+'">'
      ...
      +'<td class="xl-td-total" id="total-'+s.id+'">â</td>'
      +'<td class="xl-td-baho" id="letter-'+s.id+'">â</td>'
      +'<td style="text-align:center"><button class="xl-save-btn" onclick="saveGrade('+s.id+')">ð¾ Saqlash</button></td>'
      +'</tr>';
  }).join('');
...
function updateGradeFooter(students){
  ...
  var avg=students.length?(sum/students.length).toFixed(1):'â';
...
function saveGrade(id){
  ...
  if(jn>30||on>20||yn>30||mi>20){showToast('â ï¸','Xato','Ball limitdan oshib ketdi!');return;}
  ...
  showToast('â','Saqlandi','Baholar muvaffaqiyatli saqlandi');
}
...
function saveAllGrades(){
  ...
  if(err){showToast('â ï¸','Xato','Limitdan oshgan baholar mavjud!');return;}
  ...
  showToast('â','Barchasi saqlandi',_curSub+' â '+_curGrp+' baholar saqlandi');
}
...
function renderDekanatGrades(){
  ...
  if(tEl)tEl.textContent='ð '+grp+' â '+(subF==='all'?'Barcha fanlar':subF);
  ...
  var avg=cnt?(sum/cnt).toFixed(1):'â';
  ...
  sEl.innerHTML='<div class="stat-card"><div class="stat-card-top"><div class="stat-card-label">Talabalar</div><div class="stat-card-icon" style="background:var(--primary-light)">ð¥</div></div><div class="stat-card-val" style="color:var(--primary)">'+students.length+'</div><div class="stat-card-change sc-flat">'+grp+'</div><div class="stat-card-bar scb-blue"></div></div>'
   +'<div class="stat-card"><div class="stat-card-top"><div class="stat-card-label">O\'rtacha ball</div><div class="stat-card-icon" style="background:var(--green-light)">ð</div></div><div class="stat-card-val" style="color:var(--green)">'+avg+'</div><div class="stat-card-change sc-flat">Barcha fanlar</div><div class="stat-card-bar scb-green"></div></div>'
   +'<div class="stat-card"><div class="stat-card-top"><div class="stat-card-label">A\'lochilar</div><div class="stat-card-icon" style="background:var(--yellow-light)">â­</div></div><div class="stat-card-val" style="color:var(--yellow)">'+alo+'</div><div class="stat-card-change sc-up">86+ ball</div><div class="stat-card-bar scb-orange"></div></div>'
   +'<div class="stat-card"><div class="stat-card-top"><div class="stat-card-label">Qoniqarsiz</div><div class="stat-card-icon" style="background:var(--red-light)">â ï¸</div></div><div class="stat-card-val" style="color:var(--red)">'+fail+'</div><div class="stat-card-change sc-down">55 dan past</div><div class="stat-card-bar scb-red"></div></div>';
}
...
function renderGradeDistribution(){
  ...
  var data=[
    {l:"A'lo (86â100)",cnt:alo,pct:Math.round(alo/total*100),c:'#16A34A'},
    {l:'Yaxshi (71â85)',cnt:yaxshi,pct:Math.round(yaxshi/total*100),c:'#1B4FD8'},
    {l:'Qoniqarli (56â70)',cnt:qoniq,pct:Math.round(qoniq/total*100),c:'#D97706'},
